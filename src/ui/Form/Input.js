...
function Input({
  ...
  const isValidSchema = useMemo(() => {
    ...
    arrRS.validate = (value) => {
      ...
      if (rules.isUsernameLogin) {
        const usernameLoginRegex =
          /^((84|03|07|08|09|01[2|6|8|9])+([0-9]{8,9})\b(?![\!\@\#\$\%\^\&\*\)\(\+\=\.\<\>\{\}\[\]\:\;\'\"\|\~\`\_\-]))|(\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3}))+$/;
        if (!usernameLoginRegex.test(value)) {
          return `${label} ${t('không đúng định dạng')}`;
        }
      }
  ...
  return (
    <div className={'form-group ' + formGroupClassName} style={{ ...others.style }}>
      {label && (
        <label>
          {label}
