...
interface SagaExport extends SagaAction<any> {
    type?: typeof types.GET_TOP_TRANSACTION_BY_ACCOUNT.REQUEST;
}
const SQL_QUERY = `
...
`;
function* getTopTransactionByAccount({ payload, callback }: SagaExport) {
    try {
        ...
        if (dataRes.lenght != 0) {
            yield put({
                type: types.GET_TOP_TRANSACTION_BY_ACCOUNT.SUCCESS,
                payload: dataRes,
            });
            callback && callback(true, dataRes ?? {});
        } else {
            yield put({
                type: types.GET_TOP_TRANSACTION_BY_ACCOUNT.FAILURE,
            });
...
export default function* watchAction() {
    yield takeLatest(types.GET_TOP_TRANSACTION_BY_ACCOUNT.REQUEST, getTopTransactionByAccount);
}
