...
import { getChangedInfo } from 'redux/actions';
...
const DetailChangedInfoMerchant: React.FC<DetailTransDrawerProps> = ({
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(
      getChangedInfo(payload, (status, res) => {
        if (status) {
          setData(res.data);
        }
        setLoading(false);
...
export default DetailChangedInfoMerchant;
