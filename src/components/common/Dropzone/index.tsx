...
interface Props {
  ...
  onDropProps?: any;
  onClickReviewImg?: (e: React.MouseEvent<HTMLDivElement>, row?: any) => void;
}
  ...
  return !checkadd ? (
    <div
      {...getRootProps({
        ...
        } ${!multiple && 'single'} ${!isFileValid && 'error'}`,
      })}>
      <input {...getInputProps()} />
    ...
    <>
      ...
      {(multiple ? file?.length > 0 : file) ? (
        multiple ? (
          <div className={`dropzone-mutil-container ${file?.length >= 4 ? 'overflowX' : ''}`}>
            {handleRenderFiles()}
          </div>
        ) : (
          <div className='card-dropzone'>
