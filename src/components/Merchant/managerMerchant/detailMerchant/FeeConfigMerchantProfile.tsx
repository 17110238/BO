...
const FeeConfigMerchantProfile: React.FC<Props> = ({
  ...
  useEffect(() => {
    sessionStorage.setItem('feeMethodSelected', methodSelected.value);
  ...
  useEffect(() => {
    formPoboFee.reset();
    formEcommerceFee.reset();
    switch (methodSelected.value) {
      case 'ecommerceFee':
        formEcommerceFee.setValue('ecommerceFeeList', [
          ...(formatValueDefaultOnData(merchantFee, 'ecommerceFeeList') || []),
        ]);

        break;
      case 'poboFee':
        formPoboFee.setValue('poboFeeList', [
          ...(formatValueDefaultOnData(merchantFee, 'poboFeeList') || []),
        ]);
        break;
      default:
        break;
    }
  }, [methodSelected, merchantFee]);
    ...
    <Collapse in={collapse || false}>
      {loading ? (
        ...
        <div className='fee-config block-section-body__form-section px-0'>
          ...
          <p className='font-italic text-primary' style={{ padding: '0 21px' }}>
            (*) Cập nhật lần cuối
          </p>

          <div className='fee-config__log-info image-profile-merchant d-flex flex-wrap'>
            <div className='inputs-group-v2 w-100'>
              <div className={`form-group flex-full form-input-textarea `}>
                <label>{t('Description')}</label>
                {merchantFee.noteInfo?.description ? (
                  <ul style={{ listStyleType: 'disc', paddingLeft: '2rem' }}>
                    <li>{merchantFee.noteInfo?.description}</li>
                  </ul>
                ) : (
                  <p className='font-italic text-light'>(Không có dữ liệu)</p>
                )}
              </div>
            </div>
            <div className='inputs-group-image inputs-group-v2' style={{ padding: '0 21px' }}>
              <div className={`form-group flex-full inputs-group-image__imgs-log-fee `}>
                <label>
                  {t('Hình ảnh')}
                  {/* <span className='text-danger'> (*)</span> */}
                </label>
                {merchantFee.noteInfo?.images?.length ? (
                  <div className='inputs-group-image__imgs-info'>
                    <div className='imgs-info__list ml-0'>
                      {merchantFee.noteInfo?.images?.map((img, index) => {
                        return (
                          <RenderImg
          ...
          <div
            className={`fee-config__footer ${
              ['APPROVED'].includes(profile?.state || '') ? 'd-none' : ''
            }`}>
