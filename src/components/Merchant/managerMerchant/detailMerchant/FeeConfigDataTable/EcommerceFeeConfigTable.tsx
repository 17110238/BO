...
const EcommerceFeeConfigTable: React.FC<Props> = ({
                  ...
                  <label className='fee-input-item__default'>
                    <input
                      ...
                      {...register(`ecommerceFeeList.${index}.gatewayFee.isDefault`, {
                        onChange: (e) => handleCheckDefault(e, index, 'gatewayFee', row?.paymentMethodId),
                      })}
                    />
                    {t('Mặc định')}
                  </label>
                  ...
                  <label className='fee-input-item__default'>
                    <input
                      ...
                      {...register(`ecommerceFeeList.${index}.fixedGatewayFee.isDefault`, {
                        onChange: (e) => handleCheckDefault(e, index, 'fixedGatewayFee', row?.paymentMethodId),
                      })}
                    />
                    {t('Mặc định')}
                  </label>
                  ...
                  <label className='fee-input-item__default'>
                    <input
                      ...
                      {...register(`ecommerceFeeList.${index}.transactionFee.isDefault`, {
                        onChange: (e) => handleCheckDefault(e, index, 'transactionFee', row?.paymentMethodId),
                      })}
                    />
                    {t('Mặc định')}
                  </label>
                  ...
                  <label className='fee-input-item__default'>
                    <input
                      ...
                      {...register(`ecommerceFeeList.${index}.fixedTransactionFee.isDefault`, {
                        onChange: (e) => handleCheckDefault(e, index, 'fixedTransactionFee', row?.paymentMethodId),
                      })}
                    />
                    {t('Mặc định')}
                  </label>
  ...
  const handleCheckDefault: any = (
    ...
    let itemDefault = 0;
    console.log('ecommerceDefaultFee', ecommerceDefaultFee, paymentMethodId, valueString)
    if (paymentMethodId) {
      const findItem = ecommerceDefaultFee.find((item) => item.paymentMethodId === paymentMethodId);
      if (findItem) {
        itemDefault = Number(findItem[valueString])
      }
    }
  ...
  }, [merchantFee, ecommerceDefaultFee]);
  ...
  useEffect(() => {
    if (checkAll !== 'INIT') {
      ...
      form.setValue('ecommerceFeeList', newFormatValue);
    }
  }, [checkAll]);
  ...
  return (
    <form onSubmit={handleSubmit(handleSubmitForm, onSubmitError)} noValidate>
      {loading ? (
        ...
        <div
          className={`table-payment cls-datatable datatable-fee-config ${screenWidth <= 768 ? 'datatable-vertical' : ''
            }`}>
          <DataTable
