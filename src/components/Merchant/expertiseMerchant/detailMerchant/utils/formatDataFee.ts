...
const ecommerceDefaultFee: DefaultFeeMerchantConfig = store.getState()?.utility?.defaultMerchantFee;
...
export const formatValueDefaultOnData = (data: FeeMerchantConfig, type: string) => {
  ...
  return cloneData[type as keyof Omit<FeeMerchantConfig, 'noteInfo'>]?.map((fee) => {
    const dataSelect = ecommerceDefaultFee[type as keyof DefaultFeeMerchantConfig]?.find(
