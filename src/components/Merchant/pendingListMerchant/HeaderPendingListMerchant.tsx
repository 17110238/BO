...
interface HeaderPendingListProps {
  isShowFilter?: boolean;
  onClickExport?: () => void;
  onClickFilter?: () => void;
}

const HeaderPendingList: React.FC<HeaderPendingListProps> = ({
  isShowFilter,
  onClickFilter,
  onClickExport,
}) => {
  ...
  return (
    <div className='header-pending-list-container'>
      <p className='header-pending-list__title disabled'>{t('Danh sách chờ duyệt')}</p>
      <div className='header-pending-list__btn-group '>
        <button
        ...
        <button
          ...
          onClick={() => router.push('danh-sach-cho-duyet/quan-ly-quy-trinh-duyet')}>
          <i className='fas fa-list-ol'></i>
          {t('Quản lý quy trình duyệt')}
        </button>
      </div>
    </div>
  );
...
export default HeaderPendingList;
