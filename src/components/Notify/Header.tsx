...
interface HeaderHistoryProps {
  ...
  toggleFilter?: () => void;
  t: (a: string) => string;
  handleHideHistoryBox: () => void;
  showHistory: boolean;
}

const HeaderHistory: React.FC<HeaderHistoryProps> = ({
  showFilter,
  toggleFilter,
  t,
  handleHideHistoryBox,
  showHistory,
}) => {
  ...
  return (
    <div className={`header-notify-history-container ${!showHistory ? 'hide' : ''}`}>
      <p className='header-notify-history__title'>{t('Lịch sử')}</p>
      <div className='header-notify-history__btn-group'>
        <button
        ...
        <button className={`filter-btn btn hide-btn`} onClick={handleHideBox}>
