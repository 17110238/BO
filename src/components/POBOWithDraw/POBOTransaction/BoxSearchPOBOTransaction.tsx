...
import { exportPoboOrder } from 'redux/actions';
...
const typePOBO = ['typePOBO', 'PAYMENT', 'SALARY', 'WITHDRAW', 'OTHER'];

const BoxSearchPOBOTransaction: React.FC<Props> = ({
  showFilter,
  handleSubmitSearch,
  handleClearForm,
  submitForm = false,
  ...
  const onSubmit: SubmitHandler<SearchParams> = (data: any, e) => {
    ...
    let payload = {
      ...
      bulkTransactionId: data.typeId === 'bulkTransactionId' ? data.search.trim() : '',
      merchantId:
        data.typeId === 'merchantId' ? (data.merchantId ? data.merchantId.toString() : '') : '',
      type: data.typeId === 'type' ? data.type : '',
    };
  ...
  const typePOBOptions = typePOBO.map((pobo) => ({
    value: pobo === 'typePOBO' ? '' : pobo,
    label: t(`${pobo}`),
  }));

  const [merchantAndTpye, setMerchantAndType] = useState<string>('');
  const [showMerchantSelected, setShowMerchantSelected] = useState<boolean>(false);
  const [showSearch, setShowSearch] = useState<boolean>(true);
  const [showTypetSelect, setShowTypeSelected] = useState<boolean>(false);

  const [maGDPlayhoder, setMaGDPlayhoder] = useState<boolean>(true);
  const [maNHPlayhoder, setMaNHPlayhoder] = useState<boolean>(false);
  const [maDTlayhoder, setMaDTlayhoder] = useState<boolean>(false);
  const [maLoPlayhoder, setMaLoPlayhoder] = useState<boolean>(false);

  useEffect(() => {
    if (merchantAndTpye === 'transactionId') {
      setMaGDPlayhoder(true);
      setMaNHPlayhoder(false);
      setMaDTlayhoder(false);
      setMaLoPlayhoder(false);
    }
    if (merchantAndTpye === 'bankTransaction') {
      setMaGDPlayhoder(false);
      setMaNHPlayhoder(true);
      setMaDTlayhoder(false);
      setMaLoPlayhoder(false);
    }
    if (merchantAndTpye === 'partnerTransaction') {
      setMaGDPlayhoder(false);
      setMaNHPlayhoder(false);
      setMaDTlayhoder(true);
      setMaLoPlayhoder(false);
    }
    if (merchantAndTpye === 'bulkTransactionId') {
      setMaGDPlayhoder(false);
      setMaNHPlayhoder(false);
      setMaDTlayhoder(false);
      setMaLoPlayhoder(true);
    }
    if (merchantAndTpye === 'merchantId') {
      setShowMerchantSelected(true);
      setShowSearch(false);
      setShowTypeSelected(false);
      return;
    } else if (merchantAndTpye !== 'merchantId') {
      setShowMerchantSelected(false);
      setShowSearch(true);
    }
    if (merchantAndTpye === 'type') {
      setShowTypeSelected(true);
      setShowSearch(false);
      setShowMerchantSelected(false);
      return;
    } else if (merchantAndTpye !== 'type') {
      setShowTypeSelected(false);
      setShowSearch(true);
    }
  }, [merchantAndTpye]);
  ...
  const exportPoboOrderFile = async () => {
    setIsLoadingExport(true);
    dispatch(exportPoboOrder({ ...filter }, (status, res) => {}));
    sub.subscribe({
      next({ data }: any) {
        ...
        dispatch(
          handleDowloadSaga({ data: `${urlData}` }, async (state, res) => {
            if (state && dataForm.accountId === accountIdLogin) {
              ...
              dispatch({
                type: types.EXPORT_POBO_ORDER.SUCCESS,
              });
    ...
    <>
      <div className='box-search-withdrawal' ref={boxSearchRef}>
        <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              render={({ field: { onChange, value }, fieldState: { error } }) => (
                <ReactSelect
                  ...
                  onChange={(e: SingleValue<any>) => {
                    onChange(e.value), setMerchantAndType(e.value);
                  }}
                />
              )}
            />
              ...
              <Controller
                ...
                render={({ field: { onChange, value }, fieldState: { error } }) => (
                  <ReactSelect
                    ...
                    defaultValue={{ value: '', label: t('typePOBO') }}
                    theme={(theme) => ({
                      ...theme,
                      borderRadius: 0,
                      colors: {
                    ...
                    options={typePOBOptions}
                    value={typePOBOptions.find((val) => val.value === value)}
                    onChange={(e: SingleValue<any>) => {
                      onChange(e.value);
                    }}
                  />
          ...
          <Form.Group as={Col} className='state' xl='2' md='6' sm='4'>
            <Controller
              control={control}
              name={'state'}
              defaultValue={''}
          ...
          <Form.Group as={Col} className='method' xl='2' md='6' sm='6'>
            <Controller
              control={control}
              name={'destination'}
              defaultValue={''}
...
export default BoxSearchPOBOTransaction;
