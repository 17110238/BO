...
export default function NapasTable({ t, data, getDataList, isLoading, ...rest }: Props) {
  const lang = localStorage.getItem('NEXT_LOCALE');
  const router = useRouter();

  const columns: TableColumn<NapasTransactionObject>[] = useMemo(
    () => [
      {
        name: t('Mã GD'),
        minWidth: '125px',
