...
interface Props {
  ...
  onClickExport?: () => void;
  t: (a: string) => string;
  onClickOpenAddModal: () => void;
}

const HeaderTemplate: React.FC<Props> = ({
  showFilter,
  toggleFilter,
  onClickOpenAddModal,
  t,
  ...
  return (
    <div className='header-template-manage-container'>
      <p className='header-template__title'>{t('Quản lý template')}</p>
      <div className='header-template__btn-group'>
        <button
        ...
        <button className='add-btn btn' onClick={() => onClickOpenAddModal()}>
