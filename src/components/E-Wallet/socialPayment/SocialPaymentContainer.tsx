...
import ReportSocialContainer from './reportSocial/ReportSocialContainer';
import SocialPayContainer from './socialPay/SocialPayContainer';
...
const SocialPaymentContainer: React.FC<Props> = ({ activeTab }) => {
  ...
  const renderTabs = useCallback(() => {
    const tabs: any[] = [];
    const tabPanels: any[] = [];
    accountInfo.scope.includes('bo.eWalletSocialPaymentBo.searchSocialPay') &&
      (tabs.push(
        <Tab key={1} onClick={() => router.push('/vi-dien-tu/quan-ly-link-thanh-toan')}>
          <span>Quản lý</span>
        </Tab>
      ),
      tabPanels.push(
        <TabPanel key={1}>
          <SocialPayContainer isShowFilter={isShowFilter} />
        </TabPanel>
      ));
    accountInfo.scope.includes('bo.eWalletSocialPaymentBo.reportSocialPay') &&
      (tabs.push(
        <Tab key={2} onClick={() => router.push('/vi-dien-tu/quan-ly-link-thanh-toan/bao-cao')}>
          <span>Báo cáo</span>
        </Tab>
      ),
      tabPanels.push(
        <TabPanel key={2}>
          <ReportSocialContainer isShowFilter={isShowFilter} />
        </TabPanel>
      ));
  ...
  return (
    <Tabs className='custom-ui-react-tab react-tabs' defaultIndex={activeTab || 0}>
      <TabList id='idTabListAc'>
        <div className='d-flex'>{tabs.tabs.map((tab) => tab)}</div>
...
export default SocialPaymentContainer;
