...
import { AddressWalletKYC, PayloadSearchKYC, stateWalletKYCEnum, WalletKYC } from 'models';
...
interface Props {
  ...
  getDataList: (
    ...
    payload: PayloadSearchKYC;
    getList: (payload: PayloadSearchKYC) => void;
  };
  ...
  onClickShowHistory: (id: WalletKYC, type?: string) => React.MouseEventHandler<HTMLDivElement>;
}
...
const DatatableApprovalMerchant: React.FC<Props> = ({
  ...
  const generateStateMC = (state?: stateWalletKYCEnum) => {
    ...
    switch (state) {
      ...
      case stateWalletKYCEnum.PENDING_IC:
  ...
  const handleErrorImage: React.ReactEventHandler<HTMLImageElement | HTMLVideoElement> = (e) => {
  ...
  const columns: TableColumn<WalletKYC>[] = useMemo(
    () => [
      ...
      },
      {
        name: t('Loại Giấy tờ'),
        minWidth: '150px',
        // <div>{row?.accountType ? t(row?.accountType) : '-'}</div>;
        cell: (row) => {
          return <div>{row?.type ? t(row?.type) : '-'}</div>;
        },
      },
        ...
        cell: (row) => {
          return row?.video ? (
            <div
              ...
              <a
                onClick={async (e) => {
                  // https://aws-payme-sandbox.s3.ap-southeast-1.amazonaws.com/2022/05/12/Wtsgv.CGs.mp4
                  swalVideoCustom(renderImageSrc(row?.video?.video, 'EMPTY'));
                }}
                className='row-img-preview'>
                <video
                  src={renderImageSrc(row?.video?.video, 'VIDEO')}
                  data-src={renderImageSrc(row?.video?.video)}
                />
              </a>
            </div>
          ) : (
          ...
          return (
            <Dropdown className='merchant-approval-manage-table-dropdown'>
              ...
              <Dropdown.Menu style={{ borderRadius: '12px' }}>
                ...
                {row?.state === 'APPROVED' && row?.identifyIC?.state === 'PENDING' && (
                  <Dropdown.Item onClick={onClickReject(row, 'MODAL_REJECT_IC')}>
                    <i className='fas fa-minus-circle fa-lg mr-2' />
                    {t('Từ chối')} IC
                  </Dropdown.Item>
                )}
                <Dropdown.Item onClick={onClickShowHistory(row, 'MODAL_HISTORY')}>
