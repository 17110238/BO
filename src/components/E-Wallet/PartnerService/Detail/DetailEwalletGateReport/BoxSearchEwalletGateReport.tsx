...
const BoxSearchEwalletGateReport: React.FC<Props> = ({
  handleSearchForm,
  title,
  filter,
  submitForm,
  ...
  const onSubmit: SubmitHandler<any> = (data, e) => {
    ...
    router.replace(`/vi-dien-tu/tai-khoan/vi-dien-tu/doi-tac/chi-tiet-${title === 'GATE_CARD' ? "card" : "topup"}?${convertToQueryString(spreadCreatedAt)}`, undefined, {
      shallow: true,
    });
  ...
  const onClearForm: MouseEventHandler<HTMLDivElement> = (e) => {
    e.preventDefault();
    router.replace(`/vi-dien-tu/tai-khoan/vi-dien-tu/doi-tac/chi-tiet-${title === 'GATE_CARD' ? "card" : "topup"}`, undefined, { shallow: true });
  };
    ...
    <>
      {
        showFilter && <div className='box-search-partner-service'>
          <Form onSubmit={handleSubmit(onSubmit)} className='d-flex row'>
            ...
            <Form.Group as={Col} xl={4} className='form-day-from-to'>
            ...
            <div className='d-flex align-items-center search-button-group col-xl-3 ml-0'>
              <button className='btn btn-primary btn-search' style={{ whiteSpace: 'nowrap' }}>
...
export default BoxSearchEwalletGateReport