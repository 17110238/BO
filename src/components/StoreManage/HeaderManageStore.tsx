...
export default function HeaderManageStore({
  isShowFilter,
  isClickFilter,
}: PropsComponent): JSX.Element {
  const { t } = useTranslation('common');
  return (
    <div className='storeManage-header'>
      <p className='storeManage-header__title'>{t('Quản lý điểm GD')}</p>
      <div className='storeManage-header__filter'>
        <button className={`filter-btn ${!isShowFilter && 'filter-hide'}`} onClick={isClickFilter}>
          <svg
            width='14'
            height='12'
