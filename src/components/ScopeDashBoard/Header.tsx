...
interface Props {
  ...
  handleShowCreateScopeModal: () => void;
}
...
const Header: React.FC<Props> = ({
  ...
  t,
  setSubmitForm,
  handleShowCreateScopeModal,
}) => {
  return (
    <div className='header-scope-dashboard-container'>
      <p className='header-scope-dashboard__title'>{t('Danh sách scope')}</p>
      <div className='header-scope-dashboard__btn-group d-flex align-items-center'>
        <div className='d-flex align-items-center mr-3 btn-deposit'>
          <button
            className='btn btn-secondary btn-search'
            onClick={() => handleShowCreateScopeModal()}>
        ...
        <button
          className={`filter-btn filter-custom btn ${showFilter ? 'btn-active' : ''}`}
