...
interface HeaderTransactionProps {
  ...
  handleExportMerchantView: (a: string) => void;
}
...
const HeaderTransaction: React.FC<HeaderTransactionProps> = ({
  ...
  return (
    <div className='header-transaction-container'>
      <p className='header-transaction__title'>{t('Quản lý giao dịch')}</p>
      <div className='header-transaction__btn-group'>
        ...
        {checkImportPayinPermission && (
          <button className='btn btn-primary btn-import' onClick={showModalPayint}>
            <img src='/assets/icon/import-icon.png' />
            Import
          </button>
        )}
        ...
        <button
          ...
          onClick={() => {
            handleExportMerchantView('EXPORT_TRANSACTION');
          }}>
